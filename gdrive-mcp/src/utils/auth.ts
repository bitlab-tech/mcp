...
export class Auth {
  ...
  async refreshToken() {
    ...
    try {
      const result: any = await this.oauth2.createToken({
  ...
  async exchangeCodeForToken(code: string, authMethod = 'body') {
  ...
  setupCallbackServer() {
    ...
    app.get('/oauth2callback', async (req: any, res: any) => {
      ...
      try {
        ...
        res.end('<html><body><h2>Authentication received. You can close this tab.</h2></body></html>');
      }
      catch (error) {
  ...
  async executeWithAuth(operation: any): Promise<any> {
    try {
      if (this.token) {
